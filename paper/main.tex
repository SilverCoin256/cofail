...
is at that null, on $1{,}228$--$1{,}373$ open models per benchmark across four benchmarks --- a
...
literature. Empirically the measured excess matches the closed form to $10^{-16}$ on all four
benchmarks. Our pre-registered dispersion statistic came out \emph{below} its null on three of
four benchmarks and above on one; its pre-registered kill condition fired and we report it as
refuted.
...
$+0.115767366$ ($+1.4\times10^{-17}$); GSM8K $+0.042311309$ ($+7.6\times10^{-17}$). Over $400$
...
\caption{Lemma~\ref{lem:alpha} predicts the measured excess exactly on all four benchmarks
...
\caption{Observed participation ratio against its exact-margin null, all four benchmarks. Read
...
so the lineage decomposition is dropped rather than estimated on a biased subset. The
multi-category result is established analytically and in simulation; a real-response analysis
must be explicit about the estimator, since the pooled agreement ratio is composition-invariant
but the pair-averaged form is not.
