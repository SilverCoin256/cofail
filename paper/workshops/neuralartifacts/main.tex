...
identification convention. That matters here because the alternative, fitting an item-response
...
shared behaviour and is not. The excess over the composition-preserving null implied by
...
$+0.00085$), exactly as Lemma~\ref{lem:fleiss} predicts. The \emph{relationship} between
accuracy and agreement, though, is not a mean-level statistic and is not absorbed by the same conditioning.
...
$k=20$. The lineage signal is spread across many residual dimensions rather than carried by
...
\citep{vovkwang2021}; that is necessary here, since nested populations are as dependent as looks get. As
