...
\citep{ponocny2001}, to $1{,}228$--$1{,}362$ open models per benchmark, and --- unlike prior uses
--- we characterise the test's power rather than asserting its validity. Three results follow.
The residual correlation that survives conditioning is $2.8$--$11.8\times$ its null and is
\emph{not} a duplicate-model artifact: removing every model agreeing with a retained model above
$0.95$ discards $21$--$64\%$ of each population and moves $14$ of $15$ headline statistics by at
most $1.5\times$. The test has power $1.00$ against planted shared failure modes at a strength well
...
for item difficulty and $O=\frac{1}{MN(N-1)}\sum_{i\neq j}\sum_m F_{im}F_{jm}$ for mean pairwise
co-failure.
...
reproduces the both-margin null to within $8.3\%$ on every benchmark ($0.4\%$ on ARC). Empirically
the item margins do essentially all the conditioning work.
...
\section{What survives, on 1,228--1,362 models per benchmark}
...
not hold for the population-level statistics: $14$ of $15$ statistic--benchmark pairs move by at
most $1.5\times$ between the full and $0.95$-deduplicated populations (the exception is
...
HellaSwag), stable to within one across deduplication thresholds on three of five benchmarks
(Winogrande, TruthfulQA, GSM8K); ARC itself ranges $4$--$6$. These
...
dated amendments are archived at \texttt{doi:10.5281/zenodo.21860005} and \texttt{osf.io/cmh7q};
every number above traces
to an executed run.
